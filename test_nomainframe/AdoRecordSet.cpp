...
CAdoRecordSet::CAdoRecordSet()
{
	...
	m_p_record_set.CreateInstance("ADODB.Recordset");
	#ifdef _DEBUG
	if (m_p_record_set == NULL)
	{
	//	AfxMessageBox("RecordSet 对象创建失败! 请确认是否初始化了COM环境.");
		TRACE("RecordSet 对象创建失败! 请确认是否初始化了COM环境.\n");
	}
	#endif
	ASSERT(m_p_record_set != NULL);
}

CAdoRecordSet::CAdoRecordSet(CAdoConnection *pConnection)
{
	m_SearchDirection = adSearchForward;
	m_p_connection = pConnection;
...
BOOL CAdoRecordSet::Open(LPCTSTR strSQL, long lOption, CursorTypeEnum CursorType, LockTypeEnum LockType)
{
	...
	if (m_p_connection == NULL || m_p_record_set == NULL)
	{
		return FALSE;
	}
	...
	{
		if (IsOpen()) Close();
		return SUCCEEDED(m_p_record_set->Open(_variant_t(LPCTSTR(m_strSQL)), 
											_variant_t((IDispatch*)m_p_connection->GetConnection(), true),
...
BOOL CAdoRecordSet::Requery(long Options)
{
	...
	try
	{
		if (m_p_record_set != NULL) 
		{
			return (m_p_record_set->Requery(Options) == S_OK);
		}
	}
	catch (_com_error e)
	{
...
BOOL CAdoRecordSet::Resync(AffectEnum AffectRecords, ResyncEnum ResyncValues)
{
	...
	try
	{
		if (m_p_record_set != NULL) 
		{
			return (m_p_record_set->Resync(AffectRecords, ResyncValues) == S_OK);
		}
	}
	catch (_com_error e)
	{
...
BOOL CAdoRecordSet::Save(LPCTSTR strFileName, PersistFormatEnum PersistFormat)
{
	...
	try
	{
		if (m_p_record_set != NULL) 
		{
			return (m_p_record_set->Save(_bstr_t(strFileName), PersistFormat) == S_OK);
		}
	}
	catch (_com_error e)
	{
...
BOOL CAdoRecordSet::Load(LPCTSTR strFileName)
{
	...
	try
	{
		return (m_p_record_set->Open(strFileName, "Provider=MSPersist;", adOpenForwardOnly, adLockOptimistic, adCmdFile) == S_OK);
	}
	catch (_com_error &e)
	{
...
BOOL CAdoRecordSet::Cancel()
{
	...
	try
	{
		return SUCCEEDED(m_p_record_set->Cancel());
	}
	catch (_com_error e)
	{
...
void CAdoRecordSet::Close()
{
	try
	{
		if (m_p_record_set != NULL && m_p_record_set->State != adStateClosed)
		{
			if (GetEditMode() == adEditNone) CancelUpdate();
			m_p_record_set->Close();
		}
	}
...
BOOL CAdoRecordSet::UpdateBatch(AffectEnum AffectRecords)
{
	...
	try
	{
		if (m_p_record_set != NULL) 
		{
			return (m_p_record_set->UpdateBatch(AffectRecords) == S_OK);
		}
	}
	catch (_com_error e)
	{
...
BOOL CAdoRecordSet::CancelUpdate()
{
	...
	try
	{
		if (m_p_record_set != NULL) 
		{
			if (GetEditMode() == adEditNone || m_p_record_set->CancelUpdate() == S_OK)
			{
				return TRUE;
			}
		}
...
BOOL CAdoRecordSet::CancelBatch(AffectEnum AffectRecords)
{
	...
	try
	{
		if (m_p_record_set != NULL) 
		{
			return (m_p_record_set->CancelBatch(AffectRecords) == S_OK);
		}
	}
	catch (_com_error e)
	{
...
BOOL CAdoRecordSet::Delete(AffectEnum AffectRecords)
{
	...
	try
	{
		if (m_p_record_set != NULL) 
		{
			return (m_p_record_set->Delete(AffectRecords) == S_OK);
		}
	}
	catch (_com_error e)
	{
...
BOOL CAdoRecordSet::MoveFirst()
{
	...
	try
	{
		if (m_p_record_set != NULL) 
		{
			return SUCCEEDED(m_p_record_set->MoveFirst());
		}
	}
	catch (_com_error e)
	{
...
BOOL CAdoRecordSet::MoveLast()
{
	...
	try
	{
		if (m_p_record_set != NULL) 
		{
			return SUCCEEDED(m_p_record_set->MoveLast());
		}
	}
	catch (_com_error e)
	{
...
BOOL CAdoRecordSet::MovePrevious()
{
	...
	try
	{
		if (m_p_record_set != NULL) 
		{
			return SUCCEEDED(m_p_record_set->MovePrevious());
		}
	}
	catch (_com_error e)
	{
...
BOOL CAdoRecordSet::MoveNext()
{
	...
	try
	{
		if (m_p_record_set != NULL) 
		{
			return SUCCEEDED(m_p_record_set->MoveNext());
		}
	}
	catch (_com_error e)
	{
...
BOOL CAdoRecordSet::Move(long lRecords, _variant_t Start)
{
	...
	try
	{
		if (m_p_record_set != NULL) 
		{
			return SUCCEEDED(m_p_record_set->Move(lRecords, _variant_t(Start)));
		}
	}
	catch (_com_error e)
	{
...
long CAdoRecordSet::GetState()
{
	...
	try
	{
		return m_p_record_set->GetState();
	}
	catch (_com_error e)
	{
...
BOOL CAdoRecordSet::IsOpen()
{
	try
	{
		return (m_p_record_set != NULL && (GetState() & adStateOpen));
	}
	catch (_com_error e)
	{
...
long CAdoRecordSet::GetStatus()
{
	...
	try
	{
		return m_p_record_set->GetStatus();
	}
	catch (_com_error e)
	{
...
long CAdoRecordSet::GetMaxRecordCount()
{
	...
	try
	{
		return m_p_record_set->GetMaxRecords();
	}
	catch (_com_error e)
	{
...
BOOL CAdoRecordSet::SetMaxRecordCount(long count)
{
	...
	try
	{
		m_p_record_set->PutMaxRecords(count);
		return TRUE;
	}
	catch (_com_error e)
	{
...
EditModeEnum CAdoRecordSet::GetEditMode()
{
	...
	try
	{
		if (m_p_record_set != NULL) 
		{
			return m_p_record_set->GetEditMode();
		}
	}
	catch (_com_error e)
	{
...
long CAdoRecordSet::GetPageCount()
{
	...
	try
	{
		return m_p_record_set->GetPageCount();
	}
	catch (_com_error &e)
	{
...
BOOL CAdoRecordSet::SetCacheSize(const long &lCacheSize)
{
	...
	try
	{
		if (m_p_record_set != NULL && !(GetState() & adStateExecuting))
		{
			m_p_record_set->PutCacheSize(lCacheSize);
		}
	}
	catch (const _com_error& e)
	{
...
long CAdoRecordSet::GetPageSize()
{
	...
	try
	{
		return m_p_record_set->GetPageSize();
	}
	catch (_com_error &e)
	{
		TRACE(_T("Warning: GetPageCount 方法发生异常. 错误信息: %s; 文件: %s; 行: %d\n"), e.ErrorMessage(), __FILE__, __LINE__);
...
BOOL CAdoRecordSet::SetAbsolutePage(int nPage)
{
	...
	try
	{
		m_p_record_set->PutAbsolutePage((enum PositionEnum)nPage);		
		return TRUE;
	}
	catch(_com_error &e)
	{
...
long CAdoRecordSet::GetAbsolutePage()
{
	...
	try
	{
		return m_p_record_set->GetAbsolutePage();
	}
	catch(_com_error &e)
	{
...
BOOL CAdoRecordSet::SetAbsolutePosition(int nPosition)
{
	...
	try
	{
		m_p_record_set->PutAbsolutePosition((enum PositionEnum)nPosition);		
		return TRUE;
	}
	catch(_com_error &e)
	{
...
long CAdoRecordSet::GetAbsolutePosition()
{
	...
	try
	{
		return m_p_record_set->GetAbsolutePosition();
	}
	catch(_com_error &e)
	{
...
BOOL CAdoRecordSet::SetCursorLocation(CursorLocationEnum CursorLocation)
{
	...
	try
	{
		m_p_record_set->PutCursorLocation(CursorLocation);
		return TRUE;
	}
	catch (_com_error e)
	{
...
CursorLocationEnum CAdoRecordSet::GetCursorLocation()
{
	...
	try
	{
		return m_p_record_set->GetCursorLocation();
	}
	catch (_com_error e)
	{
...
BOOL CAdoRecordSet::SetCursorType(CursorTypeEnum CursorType)
{
	...
	try
	{
		m_p_record_set->PutCursorType(CursorType);
		return TRUE;
	}
	catch (_com_error e)
	{
...
CursorTypeEnum CAdoRecordSet::GetCursorType()
{
	...
	try
	{
		return m_p_record_set->GetCursorType();
	}
	catch (_com_error e)
	{
...
FieldsPtr CAdoRecordSet::GetFields()
{
	...
	try
	{
		return m_p_record_set->GetFields();
	}
	catch (_com_error e)
	{
...
CString CAdoRecordSet::GetFieldName(long lIndex)
{
	...
	try
	{
		strFieldName = LPCTSTR(m_p_record_set->Fields->GetItem(_variant_t(lIndex))->GetName());
	}
	catch (_com_error e)
	{
...
long CAdoRecordSet::GetFieldAttributes(long lIndex)
{
	...
	try
	{
		return m_p_record_set->Fields->GetItem(_variant_t(lIndex))->GetAttributes();
	}
	catch (_com_error e)
	{
...
long CAdoRecordSet::GetFieldAttributes(LPCTSTR lpszFieldName)
{
	...
	try
	{
		return m_p_record_set->Fields->GetItem(_variant_t(lpszFieldName))->GetAttributes();
	}
	catch (_com_error e)
	{
		TRACE(_T("Warning: GetFieldAttributes 方法发生异常. 错误信息: %s; 文件: %s; 行: %d\n"), e.ErrorMessage(), __FILE__, __LINE__);
...
long CAdoRecordSet::GetFieldDefineSize(long lIndex)
{
	...
	try
	{
		return m_p_record_set->Fields->GetItem(_variant_t(lIndex))->DefinedSize;
	}
	catch (_com_error e)
	{
...
long CAdoRecordSet::GetFieldDefineSize(LPCTSTR lpszFieldName)
{
	...
	try
	{
		return m_p_record_set->Fields->GetItem(_variant_t(lpszFieldName))->DefinedSize;
	}
	catch (_com_error e)
	{
		TRACE(_T("Warning: GetDefineSize 方法发生异常. 错误信息: %s; 文件: %s; 行: %d\n"), e.ErrorMessage(), __FILE__, __LINE__);
...
long CAdoRecordSet::GetFieldActualSize(long lIndex)
{
	...
	try
	{
		return m_p_record_set->Fields->GetItem(_variant_t(lIndex))->ActualSize;
	}
	catch (_com_error e)
	{
...
long CAdoRecordSet::GetFieldActualSize(LPCTSTR lpszFieldName)
{
	...
	try
	{
		return m_p_record_set->Fields->GetItem(_variant_t(lpszFieldName))->ActualSize;
	}
	catch (_com_error e)
	{
		TRACE(_T("Warning: GetFieldActualSize 方法发生异常. 错误信息: %s; 文件: %s; 行: %d\n"), e.ErrorMessage(), __FILE__, __LINE__);
...
DataTypeEnum CAdoRecordSet::GetFieldType(long lIndex)
{
	...
	try 
	{
		return m_p_record_set->Fields->GetItem(_variant_t(lIndex))->GetType();
	}
	catch (_com_error e)
	{
...
DataTypeEnum CAdoRecordSet::GetFieldType(LPCTSTR lpszFieldName)
{
	...
	try 
	{
		return m_p_record_set->Fields->GetItem(_variant_t(lpszFieldName))->GetType();
	}
	catch (_com_error e)
	{
...
BOOL CAdoRecordSet::IsFieldNull(LPCTSTR lpFieldName)
{
	try
	{
		_variant_t vt = m_p_record_set->Fields->GetItem(lpFieldName)->Value;
...
BOOL CAdoRecordSet::IsFieldNull(long index)
{
	try
	{
		_variant_t vt = m_p_record_set->Fields->GetItem(_variant_t(index))->Value;
		return (vt.vt == VT_NULL);
	}
	catch (_com_error e)
	{
...
BOOL CAdoRecordSet::PutCollect(long index, const _variant_t &value)
{
	...
	try
	{
		if (m_p_record_set != NULL) 
		{
			m_p_record_set->PutCollect(_variant_t(index), value);
			return	TRUE;
		}
	}
	catch (_com_error e)
...
BOOL CAdoRecordSet::PutCollect(LPCTSTR strFieldName, const _variant_t &value)
{
	...
	try
	{
		if (m_p_record_set != NULL) 
		{
			m_p_record_set->put_Collect(_variant_t(strFieldName), value);
			return TRUE;
		}
	}
	catch (_com_error e)
...
BOOL CAdoRecordSet::GetCollect(long index, COleDateTime &value)
{
	...
	try
	{
		value = ConvertHelp::ConvertToDate(m_p_record_set->GetCollect(_variant_t(index)));
		return TRUE;
	}
	catch (_com_error e)
	{
...
BOOL CAdoRecordSet::GetCollect(LPCTSTR strFieldName, COleDateTime &value)
{
	...
	try
	{
		value = ConvertHelp::ConvertToDate(m_p_record_set->GetCollect(_variant_t(strFieldName)));
		return TRUE;
	}
	catch (_com_error e)
	{
...
BOOL CAdoRecordSet::GetCollect(long index, COleCurrency &value)
{
	...
	try
	{
		value = ConvertHelp::ConvertToCurrency(m_p_record_set->GetCollect(_variant_t(index)));
		return TRUE;
	}
	catch (_com_error e)
	{
...
BOOL CAdoRecordSet::GetCollect(LPCTSTR strFieldName, COleCurrency &value)
{
	...
	try
	{
		value = ConvertHelp::ConvertToCurrency(m_p_record_set->GetCollect(_variant_t(strFieldName)));
		return TRUE;
	}
	catch (_com_error e)
	{
...
BOOL CAdoRecordSet::GetCollect(long index,  bool &value)
{
	...
	try
	{
		value = ConvertHelp::ConvertToBool(m_p_record_set->GetCollect(_variant_t(index)));
		return TRUE;
	}
	catch (_com_error e)
	{
...
BOOL CAdoRecordSet::GetCollect(LPCTSTR strFieldName,  bool &value)
{
	...
	try
	{
		value = ConvertHelp::ConvertToBool(m_p_record_set->GetCollect(_variant_t(strFieldName)));
		return TRUE;
	}
	catch (_com_error e)
	{
...
BOOL CAdoRecordSet::GetCollect(long index,  BYTE &value)
{
	...
	try
	{
		value = ConvertHelp::ConvertToBYTE(m_p_record_set->GetCollect(_variant_t(index)));
		return TRUE;
	}
	catch (_com_error e)
	{
...
BOOL CAdoRecordSet::GetCollect(LPCTSTR strFieldName,  BYTE &value)
{
	...
	try
	{
		value = ConvertHelp::ConvertToBYTE(m_p_record_set->GetCollect(_variant_t(strFieldName)));
		return TRUE;
	}
	catch (_com_error e)
	{
...
BOOL CAdoRecordSet::GetCollect(long index,  short &value)
{
	...
	try
	{
		value = ConvertHelp::ConvertToShort(m_p_record_set->GetCollect(_variant_t(index)));
		return TRUE;
	}
	catch (_com_error e)
	{
...
BOOL CAdoRecordSet::GetCollect(LPCTSTR strFieldName,  short &value)
{
	...
	try
	{
		value = ConvertHelp::ConvertToShort(m_p_record_set->GetCollect(_variant_t(strFieldName)));
		return TRUE;
	}
	catch (_com_error e)
	{
...
BOOL CAdoRecordSet::GetCollect(long index,  int &value)
{
	...
	try
	{
		value = (int)ConvertHelp::ConvertToLong(m_p_record_set->GetCollect(_variant_t(index)));
		return TRUE;
	}
	catch (_com_error e)
	{
...
BOOL CAdoRecordSet::GetCollect(LPCTSTR strFieldName,  int &value)
{
	...
	try
	{
		value = (int)ConvertHelp::ConvertToLong(m_p_record_set->GetCollect(_variant_t(strFieldName)));
		return TRUE;
	}
	catch (_com_error e)
	{
...
BOOL CAdoRecordSet::GetCollect(long index,  long &value)
{
	...
	try
	{
		value = ConvertHelp::ConvertToLong(m_p_record_set->GetCollect(_variant_t(index)));
		return TRUE;
	}
	catch (_com_error e)
	{
...
BOOL CAdoRecordSet::GetCollect(LPCTSTR strFieldName,  long &value)
{
	...
	try
	{
		value = ConvertHelp::ConvertToLong(m_p_record_set->GetCollect(_variant_t(strFieldName)));
		return TRUE;
	}
	catch (_com_error e)
	{
...
BOOL CAdoRecordSet::GetCollect(long index,  DWORD &value)
{
	...
	try
	{
		_variant_t result = m_p_record_set->GetCollect(_variant_t(index));
		switch (result.vt)
		{
		case VT_UI4:
		case VT_I4:
...
BOOL CAdoRecordSet::GetCollect(LPCTSTR strFieldName,  DWORD &value)
{
	...
	try
	{
		_variant_t result = m_p_record_set->GetCollect(_variant_t(strFieldName));
		switch (result.vt)
		{
		case VT_UI4:
		case VT_I4:
...
BOOL CAdoRecordSet::GetCollect(long index,  double &value)
{
	...
	try
	{
		value = ConvertHelp::ConvertToFloat(m_p_record_set->GetCollect(_variant_t(index)));
		return TRUE;
	}
	catch (_com_error e)
	{
...
BOOL CAdoRecordSet::GetCollect(LPCTSTR strFieldName,  double &value)
{
	...
	try
	{
		value = ConvertHelp::ConvertToFloat(m_p_record_set->GetCollect(_variant_t(strFieldName)));
		return TRUE;
	}
	catch (_com_error e)
	{
...
BOOL CAdoRecordSet::GetCollect(long index, CString& strValue)
{
	...
	try
	{
		...
		if (m_p_record_set->adoEOF)
		{
			return FALSE;
		}
		_variant_t value = m_p_record_set->GetCollect(_variant_t(index));
		strValue = ConvertHelp::ConvertToCString(value);
		return TRUE;
	}
	catch (_com_error e)
	{
...
BOOL CAdoRecordSet::GetCollect(LPCTSTR strFieldName, CString &strValue)
{
	...
	//		MessageBox(NULL, _T("数据库可能已经断开,\r\n请重新连接、然后重试."), _T("提示"), MB_ICONINFORMATION);
		...
		_variant_t value = m_p_record_set->GetCollect(_variant_t(LPCTSTR(strFieldName)));
		strValue = ConvertHelp::ConvertToCString(value);
		return TRUE;	
	}
	catch (_com_error e)
...
{		
	...
	if ((adFldLong & GetFieldAttributes(strFieldName)) && size > 0)
	{
		...
		if (GetChunk(GetField(strFieldName), (LPVOID)lpData))
		{
			...
			//strncpy((LPTSTR)&bmpHeader, (LPTSTR)lpData, bmfHeaderLen);
			strncpy_s((LPSTR)&bmpHeader, bmfHeaderLen, (LPSTR)lpData, bmfHeaderLen);
...
_RecordsetPtr CAdoRecordSet::operator =(_RecordsetPtr &pRecordSet)
{
	...
	{
		m_p_record_set = NULL;
		m_p_record_set = pRecordSet;
		return m_p_record_set;
	}
	return NULL;
}
...
BOOL CAdoRecordSet::Supports(CursorOptionEnum CursorOptions)
{
	...
	try
	{
		if (m_p_record_set != NULL)
		{
			return m_p_record_set->Supports(CursorOptions);
		}
	}
	catch (const _com_error& e)
	{
...
BOOL CAdoRecordSet::SetFilter(LPCTSTR lpszFilter)
{
	...
	try
	{
		m_p_record_set->PutFilter(lpszFilter);
		return TRUE;
	}
	catch (_com_error e)
	{
...
BOOL CAdoRecordSet::SetSort(LPCTSTR lpszCriteria)
{
	...
	try
	{
		m_p_record_set->PutSort(lpszCriteria);
		return TRUE;
	}
	catch (_com_error e)
	{
...
_variant_t CAdoRecordSet::GetBookmark()
{
	...
	try
	{
		if (IsOpen())
		{
			m_varBookmark = m_p_record_set->GetBookmark();
...
BOOL CAdoRecordSet::SetBookmark(_variant_t varBookMark)
{
	...
	try
	{
		...
		{
			m_p_record_set->PutBookmark(varBookMark);
			return TRUE;
		}	
	}
	catch (_com_error e)
...
CString CAdoRecordSet::GetLastError()
{
	ASSERT(m_p_connection != NULL);
	if( m_p_connection )
	return m_p_connection->GetLastErrorText();
...
BOOL CAdoRecordSet::Clone(CAdoRecordSet &pRecordSet)
{
	...
	try
	{
		pRecordSet = m_p_record_set->Clone(adLockUnspecified);
		return TRUE;
	}
	catch (_com_error e)
	{
...
BOOL CAdoRecordSet::Find(LPCTSTR lpszFind, SearchDirectionEnum SearchDirection)
{
	...
	try
	{
		...
		m_p_record_set->Find(_bstr_t(m_strFind), 0, SearchDirection, "");
...
BOOL CAdoRecordSet::FindNext()
{
	...
	try
	{
		...
		m_p_record_set->Find(_bstr_t(m_strFind), 1, m_SearchDirection, m_varBookmark);
...
BOOL CAdoRecordSet::RecordBinding(CADORecordBinding &pAdoRecordBinding)
{
	...
	try
	{
		if (SUCCEEDED(m_p_record_set->QueryInterface(__uuidof(IADORecordBinding), (LPVOID*)&m_pAdoRecordBinding)))
		{
...
int CAdoRecordSet::GetFieldIndex(CString filedName)
{
	long colCount = GetFieldsCount();
	int i = 0;
	for (i = 0 ;i < colCount; i++)
	{
		if (filedName == GetFieldName(i))
		{
			break;
		}
	}
...
/*========================================================================
	Name:	获取记录集合中指定列的字符串形式的值
	-----------------------------------------------------
	params:	index   列在集合中的位置
	        lockCursorPos   为TRUE时，保持游标原来的位置不变
			maxRows    一次返回记录的最大行
	---------------------------------
	remarks: 如果lockCursorPos为TRUE时,保持游标原来的位置不变,所以重复调用该函数返回
	         内容一样。考虑到如果记录集合中记录数较多，array数组很大，会占用较多的系统
			 资源，所以对它进行优化：
			     当lockCursorPos 为FALSE 时，将从当前记录开始，返回maxRows条记录的值。
==========================================================================*/
BOOL CAdoRecordSet::GetFieldValues(CStringArray *array, int index ,BOOL lockCursorPos,int maxRows)
{
	BOOL flag(TRUE);
	long pos ;
	long count = m_p_record_set->GetRecordCount();
	long colCount = GetFieldsCount();
	if (count <=0 || index >= colCount || index <0) //check values
	{
     #ifdef _DEBUG
		if (count <=0 )
			TRACE(_T("记录集合为空"));
...
//	m_p_record_set->MoveFirst();
	...
	{
		CString temp;
		if (!GetCollect(index,temp))
		{
			flag = FALSE;
			TRACE(_T("CAdoRecordSet::GetFieldValues 失败--%s"),GetLastError());
			break;
		}
		array->Add(temp);
		if (m_p_record_set->adoEOF)
	...
	{
		m_p_record_set->MoveFirst();
		m_p_record_set->Move(pos);
	}
	return flag;
}


BOOL CAdoRecordSet::GetFieldValues(CStringArray *array, CString fieldName,BOOL lockCursorPos ,int maxRows)

{
	int index =GetFieldIndex(fieldName);
	return GetFieldValues(array,index,lockCursorPos,maxRows);
}
...
/*========================================================================
	...
	remarks: 如果lockCursorPos为TRUE时,保持游标原来的位置不变,所以重复调用该函数返回
			 ...
			 资源，所以对它进行优化：
			     当lockCursorPos 为FALSE 时，将从当前记录开始，返回maxRows条记录的值。
==========================================================================*/
int CAdoRecordSet::GetFieldValues(int *Array, int size, int index, BOOL lockCursorPos, int maxRows)
{
	long pos ;
	long count = m_p_record_set->GetRecordCount();
	long colCount = GetFieldsCount();
...
int CAdoRecordSet::GetFieldValues(int *Array, int size, CString fieldName, BOOL lockCursorPos, int maxRows)
{
	int index =GetFieldIndex(fieldName);
	return GetFieldValues(Array,size,index,lockCursorPos,maxRows);
}
...
/*========================================================================
	...
	remarks: 如果lockCursorPos为TRUE时,保持游标原来的位置不变,所以重复调用该函数返回
			 ...
			 资源，所以对它进行优化：
			     当lockCursorPos 为FALSE 时，将从当前记录开始，返回maxRows条记录的值。
==========================================================================*/
int CAdoRecordSet::GetFieldValues(double *Array, int size, int index, BOOL lockCursorPos, int maxRows)
{
	long pos ;
	long count = m_p_record_set->GetRecordCount();
	long colCount = GetFieldsCount();
...
int CAdoRecordSet::GetFieldValues(double *Array, int size, CString fieldName, BOOL lockCursorPos, int maxRows)
{
	int index =GetFieldIndex(fieldName);
	return GetFieldValues(Array,size,index,lockCursorPos,maxRows);
}
...
/*========================================================================
	...
	remarks: 如果lockCursorPos为TRUE时,保持游标原来的位置不变,所以重复调用该函数返回
			 ...
			 资源，所以对它进行优化：
			     当lockCursorPos 为FALSE 时，将从当前记录开始，返回maxRows条记录的值。
==========================================================================*/
int CAdoRecordSet::GetFieldValues(COleDateTime *Array, int size, int index, BOOL lockCursorPos, int maxRows)
{
	long pos ;
	long count = m_p_record_set->GetRecordCount();
	long colCount = GetFieldsCount();
...
int CAdoRecordSet::GetFieldValues(COleDateTime *Array, int size, CString fieldName, BOOL lockCursorPos, int maxRows)
{
	int index =GetFieldIndex(fieldName);
	return GetFieldValues(Array,size,index,lockCursorPos,maxRows);
}
...
BOOL CAdoRecordSet::SeekToFieldValue(int fieldIndex, const CString & Value ,BOOL flag)
{
	if (flag)
	{
		MoveLast();
		...
		while (!IsBOF())
		{
			GetCollect(fieldIndex, Fieldvalue);
			if (Value == Fieldvalue)
				return TRUE;
			MovePrevious();
		}
...
BOOL CAdoRecordSet::LoadOLEDataFromDB(CString CreateFileName ,CString feildName)
{
